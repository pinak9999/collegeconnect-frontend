...
import io from "socket.io-client"; // 🚀 NAYA (STEP 4)
import { useAuth } from "../context/AuthContext"; // 🚀 NAYA (STEP 4)
...
// ===============================
const globalStyles = `
/* ... (Aapka CSS code jaisa tha waisa hi - yahaan brevity ke liye hide kiya gaya hai) ... */
...
:root{--bg: #f4fcf6ff; --card: #ffffff; --panel: #ffffffcc; --stroke: #e5e7eb; --txt: #1f2937; --muted: #6b7280; --brand1: #7c3aed; --brand2: #06b6d4; --ok: #16a34a; --warn: #f59e0b; --danger: #ef4444; --bg-gradient: linear-gradient(180deg, #f4f7fc 0%, #f4f7fc 100%); --panel-bg: #ffffffcc; --input-bg: linear-gradient(180deg, #ffffff 0%, #f9fafb 100%); --tab-bg: #f3f4f6; --tab-hover-bg: #e5e7eb; --info-bg: #eff6ff; --info-color: #3b82f6; --rating-bg: #f9fafb; --rating-prompt-color: var(--brand1); --rated-bg: #f0fdf4; --rated-color: #166534; --dispute-color: #fca5a5; --modal-bg: #ffffff;}
.dark {--bg: #0b1220; --card: #0f172a99; --panel: #101827cc; --stroke: #27324a; --txt: #e5e7eb; --muted: #9aa3b2; --brand1: #7c3aed; --brand2: #06b6d4; --ok: #16a34a; --warn: #f59e0b; --danger: #ef4444; --bg-gradient: linear-gradient(180deg, #050816 0%, #0b1220 100%); --panel-bg: #101827cc; --input-bg: linear-gradient(180deg, #0b1220 0%, #0b1220 60%, #0d1628 100%); --tab-bg: linear-gradient(180deg,#0e1628,#0b1323); --tab-hover-bg: linear-gradient(180deg,#0e1628,#0b1323); --info-bg: linear-gradient(180deg, #0e1b31, #0b1426); --info-color: #c7d2fe; --rating-bg: linear-gradient(180deg, #0e1628, #0b1323); --rating-prompt-color: #c7d2fe; --rated-bg: linear-gradient(180deg, #0d1a2f, #0a1324); --rated-color: #86efac; --dispute-color: #fca5a5; --modal-bg: linear-gradient(145deg, #0f172a, #0b1220);}
* { outline: none; box-sizing: border-box; }
.page-bg { min-height: 100vh; background: radial-gradient(1000px 400px at -10% -10%, rgba(124,58,237,.25), transparent 60%), radial-gradient(800px 350px at 110% 0%, rgba(6,182,212,.22), transparent 60%), var(--bg-gradient); background-color: var(--bg); font-family: "Poppins", sans-serif;}
...
.cc-input, .cc-select { border:1px solid var(--stroke); background: var(--input-bg); color:var(--txt); border-radius:14px; padding:12px 16px; width:100%; transition:.25s border-color, .25s box-shadow, .25s transform; font-family: "Poppins", sans-serif; font-size: auto;}
...
const SkeletonCard = () => (
  <div className="card" style={{ padding: 18 }}>
    ...
    <div
      className="skeleton"
      style={{ width: "60%", height: 14, margin: "14px auto" }}
    />
    <div
      className="skeleton"
      style={{ width: "40%", height: 12, margin: "8px auto" }}
    />
    <div
      className="skeleton"
      style={{ width: "80%", height: 10, margin: "16px auto" }}
    />
    <div
      className="skeleton"
      style={{ width: "50%", height: 12, margin: "16px auto" }}
    />
    <div
      className="skeleton"
      style={{
        ...
        <div style={{ display: "grid", gap: 10 }}>
          ...
          <div className="tag-chips-container">
            ...
            {tags.slice(0, 10).map((t) => (
              <Chip
                key={t._id}
                active={selectedTag === t._id}
                onClick={() => setSelectedTag(t._id)}
              >
                {t.name}
              </Chip>
            ))}
      ...
      <div className="grid-style-seniors">
        {loading
              ...
              <div key={p._id} className="card senior-card">
                ...
                <div className="rating-row rating-container">
                  {[...Array(5)].map((_, i) => (
                    <StarIcon
                      key={i}
                      filled={i < Math.round(p.average_rating || 0)}
                    />
                  ))}
                ...
                <div className="price-row">
                  <span className="price-text">
                    ₹{p.price_per_session || 0}
                  </span>
                  <span className="small-chip">
                    {p.session_duration_minutes || 20} min
                  </span>
                </div>
          ...
          : (
            ...
            >
              <div
                style={{ fontSize: 18, color: "var(--txt)", fontWeight: 700 }}
              >
                No seniors found
              </div>
...
// ===============================
const MyBookings = ({ seniors, auth }) => { // 🚀 NAYA: auth prop
  ...
  const [proposingBookingId, setProposingBookingId] = useState(null);

  // 🚀 NAYA: loadBookings ko useCallback me daalein (STEP 4)
  const loadBookings = useCallback(async () => {
    // setLoading(true); // Re-fetch pe loading state
    try {
  ...
  useEffect(() => {
    ...
    // 🚀 NAYA SOCKET.IO LOGIC (STEP 4)
    const studentId = auth?.user?.id; // Auth context se student ID lein
    if (studentId) {
      socket.emit("join_room", studentId); // Apni personal room join karein
    }

    // Senior ne time accept kiya
    socket.on("time_confirmed", (booking) => {
      toast.success("✅ Senior confirmed the time!");
      loadBookings(); // List refresh karein
    });

    // Senior ne time reject kiya
    socket.on("time_rejected", (booking) => {
      toast.error("❌ Senior rejected the time. Please propose a new time.");
      loadBookings(); // List refresh karein
    });

    // Cleanup
    return () => {
      socket.off("time_confirmed");
      socket.off("time_rejected");
      if (studentId) {
        // socket.emit("leave_room", studentId); // Optional
      }
    };
  }, [loadBookings, auth?.user?.id]); // loadBookings ab stable hai
  ...
  // 🚀 NAYA FUNCTION: Propose Time (STEP 4)
  const handleProposeTime = async (bookingId) => {
    if (!proposedTime) {
      return toast.error("Please select a time first.");
    }
    setProposingBookingId(bookingId); // Loading state
    try {
      const token = localStorage.getItem("token");
      await axios.post(
        `${API_URL}/api/bookings/propose-time/${bookingId}`,
        { time: proposedTime }, // Body me time bhejein
        { headers: { "x-auth-token": token } }
      );
      toast.success("Time proposed! Waiting for senior to accept.");
      setProposedTime("");
      loadBookings(); // List refresh karein
    } catch (e) {
      toast.error(e.response?.data?.msg || "Error proposing time");
    } finally {
  ...
  const handleRating = async () => {
    ...
    try {
      const token = localStorage.getItem("token");
      await axios.post(
        `${API_URL}/api/ratings/submit/${bookingId}`,
        { rating: value },
        { headers: { "x-auth-token": token } }
      );
  ...
  // 🚀 NAYA RENDER BOOKING CARD (STEP 4)
  // --- Ye function poora badal diya gaya hai ---
    ...
    return (
      <div key={b._id} className="card booking-card">
        ...
        <div className="status-row">
          ...
          {disputeTagClass && (
            <span className={disputeTagClass}>{b.dispute_status}</span>
          )}
        </div>

        {/* --- 🚀 NAYA TIME STATUS SECTION (STEP 4) --- */}
        <div className="status-row" style={{ marginTop: '10px', flexDirection: 'column', alignItems: 'flex-start' }}>
          <h4 style={{ margin: '0 0 8px 0', color: 'var(--txt)', fontSize: '1rem' }}>🕒 Call Time Status:</h4>
          
          {/* Case 1: Time set nahi hai */}
          {b.status_timing === "not_set" && (
            <div style={{ width: '100%' }}>
              <p className="small-muted">Please propose a time for the call.</p>
              <input 
                type="datetime-local" 
                className="cc-input"
              ...
              />
              <button 
                className="cc-btn primary btn-compact"
                onClick={() => handleProposeTime(b._id)}
                disabled={proposingBookingId === b._id}
              >
                {proposingBookingId === b._id ? "Proposing..." : "Propose Time"}
              </button>
            </div>
          )}

          {/* Case 2: Student ne propose kiya hai */}
          {b.status_timing === "student_proposed" && (
            <div className="info-message" style={{ width: '100%' }}>
              🙋‍♂️ You proposed: {new Date(b.proposed_time.student_time).toLocaleString()}
              <br/>
              <span style={{ color: 'var(--brand1)'}}>⏳ Waiting for senior to accept...</span>
            </div>
          )}
          
          {/* Case 3: Time confirm ho gaya hai */}
          {b.status_timing === "confirmed_time" && (
            <div className="rated-section" style={{ width: '100%' }}> {/* Using 'rated-section' for green BG */}
              ✅ Confirmed Time: {new Date(b.final_time).toLocaleString()}
            </div>
          )}
        </div>
        {/* --- 🚀 END TIME STATUS SECTION --- */}


        {/* Rating Section (Pehle jaisa hi) */}
        ...
        {b.rated && (
          <div className="rated-section">
            {/* ... (Rated section ka code) ... */}
          </div>
        )}

        {/* --- 🚀 NAYA BUTTON ROW LOGIC (STEP 4) --- */}
        <div className="button-row">
          {status === "confirmed" && (
            <>
              {/* Sirf tab dikhao jab time confirm ho */}
              {b.status_timing === "confirmed_time" ? (
                <Link
                  to={`/session/${b._id}`}
                  className="cc-btn primary btn-compact"
          ...
          {dispute === "none" && !b.rated && (
            <button
              className="cc-btn danger btn-compact"
          ...
          {(dispute === "not_allowed" || b.rated) &&
            dispute !== "pending" &&
            dispute !== "resolved" && (
              <span className="dispute-not-allowed">
  ...
  return (
    <div className="page-wrapper">
      ...
      {pastBookings.length > 0 && (
        <>
          <h3 className="section-title-style" style={{ marginTop: 28 }}>
            Completed & Past
          </h3>
          <div className="grid-style-bookings">
...
const StudentDashboard = () => {
  ...
  const { auth } = useAuth(); // 🚀 NAYA (STEP 4)
    ...
    const fetchAll = async () => {
      try {
        ...
        const [s, c, t] = await Promise.all([
          axios.get(`${API}/api/profile/all`, {
            headers: { "x-auth-token": token },
          }),
          axios.get(`${API}/api/colleges`, {
            headers: { "x-auth-token": token },
          }),
          axios.get(`${API}/api/tags`, {
            headers: { "x-auth-token": token },
          }),
        ]);
        setSeniors(s.data);
    ...
    <div className={`page-bg ${theme}`}>
      ...
      <div className="main-container">
        ...
        <Routes>
          ...
          <Route
            path="/bookings"
            element={<MyBookings seniors={seniors} auth={auth} />} // 🚀 NAYA: auth prop pass kiya
          />
        </Routes>
      </div>
    </div>
